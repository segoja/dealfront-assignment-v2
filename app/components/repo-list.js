...
export default class RepoListComponent extends Component {
  ...
  get filteredRepos() {
    ...
    return this.repos.filter(repo => {
      ...
      let matchLanguage = !this.language || repo.language.toLowerCase().includes(lowerCaseLanguage);
  ...
  @action
    ...
    if (this.nameError || this.tokenError) {
      ...
      let { headers, error } =  await this.githubApi.fetchRepos(this.name, this.token, 'all', 1, 1);
      ...
      let response = await this.githubApi.fetchRepos(this.name, this.token, 'all', this.currentPage);
