...
export default class RepoItem extends Component {
    ...
    async getBranches() {
      if (!this.args.token || !this.args.repo.branchesUrl) {
        ...
        this.isLoading = true;
        let response = await this.githubApi.fetchBranches(this.args.repo.branchesUrl, this.args.token);
    ...
    async getTotalBranches() {
      if (!this.args.token || !this.args.repo.branchesUrl) {
        ...
        let response = await this.githubApi.fetchBranches(this.args.repo.branchesUrl, this.args.token, 1, 1);
        this.totalBranches = this.githubApi.extractTotalPages(response.headers);
        this.isLoading = false;
      }
    }
    ...
    async loadMore() {
      if (this.currentPage <= this.totalPages) {
        ...
        this.currentPage++;
        this.isLoading = true;
        let response = await this.githubApi.fetchBranches(this.args.repo.branchesUrl, this.args.token, this.currentPage);
